...
var getBranches = function () {
    ...
    var branches = stdout.split('\n').filter(function (item) { return !!item; }).map(function (item) {
        var origin = '';
        var rest = item;
        var originBegin = item.indexOf('[origin');
        if (originBegin !== -1) {
            var originEnd = originBegin + item.slice(originBegin).indexOf(']') + 1;
            origin = item.slice(originBegin, originEnd);
            rest = item.slice(0, originBegin) + item.slice(originEnd);
        }
        var temp = rest.split(/\s+/g).map(function (property) { return property.trim(); });
        var branch = {
            current: temp.shift() === '*',
            origin: origin,
            name: temp.shift() || '',
